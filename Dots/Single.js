...
selectColor = (int) => {
  ...
  fillLine = (event) =>{
    ...
    if (this.state.lineCoordinates[currentCoord] === 0) {
      ...
      //var hist = {}
      //hist = this.state.statehistory.slice()
      let hist = JSON.parse(JSON.stringify(this.state))
      // let hist = this.state
      console.log("hist is logged here",hist)
      newState[currentCoord] = 1
      this.setState(prevState => ({
        lineCoordinates: newState,
        statehistory: hist,
        // statehistory: prevState.statehistory.concat(hist),
      }))
      console.log("statehistory",this.state.statehistory)
...
initialBoard = (size) => {
  ...
  undo = () =>{
    // this.setState((prevState) => lineCoordinates: this.state.statehistory)
    // console.log(prevState)
    // this.setState((prevState) => prevState.statehistory)
    // console.log(this.state)
    // var history = this.state.statehistory
    // console.log(history)
    // console.log("curr", this.state)

    this.setState((prevState) => this.state.statehistory)
    ...
    //   lineCoordinates: {},
    //   boxColors: boxColors
    //   statehistory: prevState.statehistory,
    // }))
  }

  restart = () =>{
    // console.log("jwwjwjw")
    freshstate = this.initialBoard(3)
    // console.log("heheeheh")
    this.setState((prevState) => freshstate)
  ...
  render() {
    ...
    return (
      <View style={styles.container}>
        ...
        <Text style={styles.text}>Player: {state.numRed}{'\t'}{'\t'} Computer: {state.numBlue}</Text>
